...
* **Vector Definition:** $E\_t \= \[P\_t, A\_t, D\_t\]$ (Range: $-1.0 \\sim \+1.0$)  
...
* 감정 감쇠(Emotional Decay):  
  이벤트가 없을 때 감정은 기준점(Baseline, 보통 0)으로 서서히 복귀한다. (기질에 따라 감쇠 속도 $\\lambda$ 조절 가능)$$E\_{t+1} \= E\_t \\times (1 \- \\lambda)$$
...
$$Score(m) \= w\_{sim} \\cdot \\text{CosineSim}(q, m) \+ w\_{emo} \\cdot \\| \\Delta E\_{m} \\|$$

* $\\| \\Delta E\_{m} \\|$: 당시 감정 변화의 크기 (충격의 강도).
...
$$E\_{eval} \= (1 \- \\alpha) \\cdot E\_{base} \+ \\alpha \\cdot \\sum\_{i=1}^{k} (Sim(m\_i) \\cdot E\_{memory\\\_i})$$
